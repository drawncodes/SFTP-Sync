...
// Turn cryptic Node/ssh2 connection errors into a short plain-language explanation
// plus a hint, so the notification says what actually happened and what to try.
function explainSSHError(err: any, host: string): string {
  ...
  switch (code) {
    case 'ECONNRESET':
      explanation =
        'connection reset by the server. It closed the connection unexpectedly — ' +
        'often rate-limiting/fail2ban after many rapid connections, a firewall, or an idle timeout. ' +
        'Try again; if it persists, lower "concurrency" in sftp.json.';
      break;
    case 'ETIMEDOUT':
      explanation =
        'connection timed out. The server did not respond — check the host/port, your network, ' +
        'or raise "connectTimeout" in sftp.json.';
      break;
    case 'ECONNREFUSED':
      explanation =
        'connection refused. Nothing is accepting connections on that port — check "port" and that ' +
        'the SSH/SFTP service is running.';
      break;
    case 'ENOTFOUND':
      explanation = 'host not found. DNS could not resolve the address — check "host" in sftp.json.';
      break;
    case 'EHOSTUNREACH':
    case 'ENETUNREACH':
      explanation = 'host unreachable. A network/routing or firewall issue is blocking the connection.';
      break;
    default:
      if (level === 'client-authentication') {
        explanation =
          'authentication failed. Check "username", and your password / private key / passphrase. ' +
          'For a key, confirm "privateKeyPath" points to the right file and the server accepts it.';
      } else if (level === 'client-timeout') {
        explanation = 'handshake timed out before the server responded. Try again or raise "connectTimeout".';
      } else {
        explanation = (err && err.message) || 'unknown connection error.';
      }
  }

  const detail = code ? ` (${code})` : '';
  return `[${host}] ${explanation}${detail}`;
}
  ...
  private async _connectSSHClient(
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      client
        .on('ready', resolve)
        .on('error', err => {
          const wrapped: any = new Error(explainSSHError(err, option.host));
          // preserve original code/level so callers can decide whether to retry
          wrapped.code = err && err.code;
          wrapped.level = err && err.level;
          reject(wrapped);
        })
        .on('close', () => this.end())
        .on('end', () => this.end())
